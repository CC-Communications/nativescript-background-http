...
net.gotev.uploadservice.UploadService.NAMESPACE =
  ...
  const ProgressReceiverImpl = net.gotev.uploadservice.UploadServiceBroadcastReceiver.extend(
    {
      ...
      onCompleted(
        ...
        task.notify({
          eventName: "responded",
          object: task,
          data: serverResponse.getBodyAsString()
        });
        task.notify({
          eventName: "complete",
          object: task,
...
export function session(id: string) {
  if (net.gotev.uploadservice.UploadService.NAMESPACE == null)
    net.gotev.uploadservice.UploadService.NAMESPACE =
      application.android.packageName;
...
class Task extends ObservableBase {
  ...
  static create(session: Session, file: string, options: common.Request): Task {
    ...
    const request = new net.gotev.uploadservice.BinaryUploadRequest(
      context,
      task._id,
      options.url
    );
    ...
    const displayNotificationProgress =
      typeof options.androidDisplayNotificationProgress === "boolean"
        ? options.androidDisplayNotificationProgress
        : true;
    if (displayNotificationProgress) {
      request.setNotificationConfig(
        new net.gotev.uploadservice.UploadNotificationConfig()
      );
    }
    const autoDeleteAfterUpload =
      typeof options.androidAutoDeleteAfterUpload === "boolean"
        ? options.androidAutoDeleteAfterUpload
        : false;
  ...
  static createMultiPart(
    ...
    const request = new net.gotev.uploadservice.MultipartUploadRequest(
      context,
      task._id,
      options.url
    );
      ...
      if (curParam.filename) {
        ...
        if (fileName.startsWith("~/")) {
          fileName = fileName.replace(
            "~/",
            fileSystemModule.knownFolders.currentApp().path + "/"
          );
        }

        const destFileName =
          curParam.destFilename ||
